...
function effectiveWindow(){
  ...
  if(state.period==='YDAY'){
    // Use per-region last date; for 'both' use UK end (the later one) for ad spend window
    const yadayEnd = state.region==='US' ? (PNL.window_end_us||end) : (PNL.window_end_uk||end);
    return {from:yadayEnd, to:yadayEnd};
  }
...
function renderKPIs(){
  ...
  const ukVatRec = (ukAdSpendIncVat + ukSmartPromoIncVat) * (20/120);
...
function renderPnLTable(){
  ...
  const ukVatRec = (ukAdInc + ukAdUnInc + ukSPinc) * (20/120);
